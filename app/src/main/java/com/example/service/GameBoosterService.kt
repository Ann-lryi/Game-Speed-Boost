...
class GameBoosterService : Service() {
    ...
    // Butler state tracking
    private var lastButlerPhase = ""
    private var butlerStatus = "Ready"

    // FPS tracking
    private var fpsTracker: SystemMetrics.FpsTracker? = null
    ...
    private fun resetSystemSettings() {
        // Only reset if Shizuku is authorized
        ...
        try {
            ...
            // Note: We intentionally don't reset refresh rate here
            // as the user may want to keep their preferred setting
        } catch (e: Exception) {
    ...
    private fun startStatsPollingLoop() {
        ...
        fpsTracker = SystemMetrics.FpsTracker()
        ...
        statsJob = serviceScope.launch(Dispatchers.IO) {
            ...
            // Delay first reading for accurate CPU delta
            delay(500)
            ...
            while (isActive) {
                try {
                    ...
                    currentFps = fpsTracker?.getCurrentFps() ?: 0
    ...
    private fun handleGameActive(
        ...
        // Dynamic thermal management based on actual temperature
        when {
            batteryTemp >= 45 -> {
                // Thermal emergency - aggressive cooling
                if (lastButlerPhase != "thermal_emergency") {
                    ...
                    if (pid > 0) {
                        executeButlerCommand("renice -n 0 -p $pid", "Reset priority")
                    }
                    repository.insertLog(
                        BoostLog(
                            actionName = "⚠️ Thermal Emergency",
                            details = "Battery temp reached ${batteryTemp}°C. Activating emergency cooling: reducing power mode, resetting process priority."
                        )
                    )
                }
            ...
            batteryTemp >= 40 -> {
                // Warm - balanced profile
                if (lastButlerPhase != "balanced") {
                    lastButlerPhase = "balanced"
                    if (pid > 0) {
                        executeButlerCommand("renice -n -5 -p $pid", "Elevated priority")
                    }
                    repository.insertLog(
                        BoostLog(
                            actionName = "⚖️ Balanced Mode",
                            details = "Battery temp ${batteryTemp}°C. Applied balanced profile with moderate priority boost."
                        )
                    )
                }
                ...
                // Cool - performance mode
                if (lastButlerPhase != "performance") {
                    lastButlerPhase = "performance"
                    executeButlerCommand("cmd power set-mode 1", "Performance mode")
                    if (pid > 0) {
                        executeButlerCommand("renice -n -10 -p $pid", "High priority")
                    }
                    repository.insertLog(
                        BoostLog(
                            actionName = "🚀 Performance Mode",
                            details = "Battery temp ${batteryTemp}°C. Applied performance profile with high priority."
                        )
                    )
                }
    ...
    private fun handleGameClosed(repository: BoosterRepository) {
        isGameActive = false
        lastButlerPhase = ""
        ...
        try {
            ...
            serviceScope.launch(Dispatchers.Main) {
                if (isGameActive && isStealthMode && targetGameName != null) {
                    ...
                    val text = if (isGameActive && targetGameName != null && currentFps > 0) {
                        "⚡ $targetGameName • ${currentFps} FPS • CPU ${if (cpuLoad >= 0) cpuLoad else "--"}% • RAM ${if (ramPercent >= 0) ramPercent else "--"}%"
                    } else {
    ...
    private fun showFloatingView() {
        ...
        try {
            removeFloatingView() // Ensure clean state
    ...
    private fun applyUfsTweaks() {
        ...
        val kbValue = when {
            ...
            else -> return // Don't apply if can't detect UFS version
        }
            ...
            for (dev in blockDevices) {
                ...
                try {
                    ...
                    if (readRes.isSuccess && readRes.output.isNotBlank()) {
                        val currentVal = readRes.output.trim()
                        if (currentVal.all { it.isDigit() }) {
                            backupReadAheadKbList[path] = currentVal
                        }
                    }
                } catch (e: Exception) {
                    // Device might not exist, skip
                }
            }
        }
        ...
        for (dev in blockDevices) {
            val path = "/sys/block/$dev/queue/read_ahead_kb"
            try {
                ShizukuManager.executeShell("echo $kbValue > $path")
                Log.d(TAG, "Applied read_ahead_kb=$kbValue for $dev")
            } catch (e: Exception) {
    ...
    private fun restoreUfsTweaks() {
        ...
        for ((path, oldVal) in backupReadAheadKbList) {
            try {
                ShizukuManager.executeShell("echo $oldVal > $path")
                Log.d(TAG, "Restored $path to $oldVal")
            } catch (e: Exception) {
                Log.e(TAG, "Error restoring $path", e)
            }
        }
